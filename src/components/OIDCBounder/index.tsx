...
export const OIDCBounder_: FC<{ children: React.ReactElement }> = ({ children }) => {
	// --- COMMENTED OUT ORIGINAL UNUSED VARIABLES DURING BYPASS ---
	// const intl = useIntl();
	...
	// };
	// --- COMMENTED OUT ORIGINAL UNUSED VARIABLES/FUNCTIONS END ---

	// --- BYPASS MOCK LOGIN START ---
	useEffect(() => {
	...
	}, [initialState?.permissionLoading]);
	// --- BYPASS MOCK LOGIN END ---

	// --- COMMENTED OUT ORIGINAL OIDC SIDE EFFECTS FOR LOCAL TESTING ---
	// useEffect(() => {
	...
	// --- COMMENTED OUT ORIGINAL OIDC SIDE EFFECTS END ---

	useEffect(() => {
		OIDCBounderHandlers = actions;
	}, [actions]);

	return <>{initialState?.permissionLoading ? <LoadingPage /> : children}</>;
};
