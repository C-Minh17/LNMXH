...
const StatisticGrdp = () => {
  ...
  const { statisticGrdpEx, statisticGrdpIn, statisticGrdpLoading, handleStatisticGrdp, handleBackgroundTask, handleExtractGrdp } = useModel("statistic.statistic")
  const [statusExtract, setStatusExtract] = useState<string>()
  const [taskId, setTaskId] = useState<string | undefined>(() => {
    return sessionStorage.getItem("task_id_grdp") || undefined;
  });

  const location = useLocation();
  const pathSegments = location.pathname.split('/');
  const type = pathSegments[2];
  const dataSource = type === 'external' ? statisticGrdpEx : statisticGrdpIn;

  const updateTaskId = (id: string | undefined) => {
    setTaskId(id);
    if (id) {
      sessionStorage.setItem("task_id_grdp", id);
    } else {
      sessionStorage.removeItem("task_id_grdp");
    }
  };

  const [openFilter, setOpenFilter] = useState(false);
  const [form] = Form.useForm();
  const [params, setParams] = useState<MStatistic.QueryParams1>({
    year: undefined,
    quarter: undefined,
  ...
  const columns: IColumn<MStatistic.IGrdp>[] = [
    {
    ...
    //   width: 80,
    //   fixed: 'right',
    //   render: (record: MStatistic.IGrdp) => {
    //     return (
    //       <>
    ...
    <>
      ...
      >
        ...
        >
          ...
          <Row gutter={16}>
            <Col span={24}>
              <Form.Item name="period_type" label="Loại kỳ dữ liệu">
                <Select placeholder="Chọn loại kỳ" allowClear>
                  ...
                  <Select.Option value="9_MONTHS">9 Tháng</Select.Option>
                </Select>
              </Form.Item>
            </Col>
          </Row>
