...
const StatisticTvetEmployment = () => {
  const { token } = theme.useToken()
  const { statisticTvetEmploymentEx, statisticTvetEmploymentIn, statisticTvetEmploymentLoading, handleStatisticTvetEmployment, handleBackgroundTask, handleExtractTvetEmployment } = useModel("statistic.statistic")
  const [statusExtract, setStatusExtract] = useState<string>()
  const [taskId, setTaskId] = useState<string | undefined>(() => {
    return sessionStorage.getItem("task_id_TvetEmployment") || undefined;
  });

  const location = useLocation();
  const pathSegments = location.pathname.split('/');
  const type = pathSegments[2];
  const dataSource = type === 'external' ? statisticTvetEmploymentEx : statisticTvetEmploymentIn;

  const updateTaskId = (id: string | undefined) => {
    setTaskId(id);
    if (id) {
      sessionStorage.setItem("task_id_TvetEmployment", id);
    } else {
      sessionStorage.removeItem("task_id_TvetEmployment");
    }
  };
  ...
  const columns: IColumn<MStatistic.ITvetEmployment>[] = [
    {
    ...
    },
    {
      ...
      width: 100,
      align: 'center',
      render: (record: MStatistic.ITvetEmployment) => {
    ...
    {
      title: "Số tốt nghiệp",
      dataIndex: "total_graduates",
      width: 140,
      align: 'right',
      render: (val: number) => (val !== null ? val.toLocaleString() : "N/A"),
    },
    {
      title: "Số có việc làm",
      dataIndex: "employed_graduates",
      width: 140,
      align: 'right',
      render: (val: number) => (val !== null ? val.toLocaleString() : "N/A"),
    },
    ...
    {
      title: "Việc đúng ngành (%)",
      dataIndex: "relevant_job_rate",
      width: 160,
      align: 'right',
      render: (val: number) => (val !== null ? `${val}%` : "N/A"),
    },
    {
      title: "Lương khởi điểm TB",
      dataIndex: "average_starting_salary",
      width: 160,
      align: 'right',
      render: (val: number) => (val !== null ? val.toLocaleString() : "N/A"),
    },
    {
      title: "Mức độ hài lòng DN (%)",
      dataIndex: "employer_satisfaction",
      width: 180,
      align: 'right',
      render: (val: number) => (val !== null ? `${val}%` : "N/A"),
    },
    {
      title: "Tuyển sinh TVET",
      dataIndex: "tvet_enrollment",
      width: 140,
      align: 'right',
      render: (val: number) => (val !== null ? val.toLocaleString() : "N/A"),
    },
    {
      title: "Số cơ sở đào tạo",
      dataIndex: "tvet_facilities",
      width: 140,
      align: 'right',
      render: (val: number) => (val !== null ? val.toLocaleString() : "N/A"),
    },
    {
      ...
      dataIndex: "industry_partnership_count",
      width: 160,
      align: 'right',
      render: (val: number) => (val !== null ? val.toLocaleString() : "N/A"),
    },
    ...
    //   width: 80,
    //   fixed: 'right',
    //   render: (record: MStatistic.IVocationalTraining) => {
    //     return (
    //       <>
  ...
  useEffect(() => {
    handleStatisticTvetEmployment(params)
  ...
  return (
    <>
      ...
      <Card
        title={<span style={{ color: token.colorPrimary }}>Thống kê việc làm sau đào tạo nghề</span>}
      ...
      >
        <TableStaticData
          ...
          data={dataSource || []}
          addStt={true}
          loading={statisticTvetEmploymentLoading}
          hasTotal
          onReload={() => handleStatisticTvetEmployment(params)}
          otherButtons={[
            <>
              <Tooltip title={statusExtract === "COMPLETED" ? "Cập nhật dữ liệu mới" : "Đang cập nhật dữ liệu"}>
                <Button
                  onClick={async () => {
                    const res = await handleExtractTvetEmployment();
                    if (res?.task_id) {
...
export default StatisticTvetEmployment