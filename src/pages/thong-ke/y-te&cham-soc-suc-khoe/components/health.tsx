...
const StatisticHealthStatistics = () => {
  const { token } = theme.useToken()
  const { statisticHealthStatisticsEx, statisticHealthStatisticsIn, statisticHealthStatisticsLoading, handleStatisticHealthStatistics, handleBackgroundTask, handleExtractHealthStatistics } = useModel("statistic.statistic")
  const [statusExtract, setStatusExtract] = useState<string>()
  const [taskId, setTaskId] = useState<string | undefined>(() => {
    return sessionStorage.getItem("task_id_HealthStatistics") || undefined;
  });

  const location = useLocation();
  const pathSegments = location.pathname.split('/');
  const type = pathSegments[2];
  const dataSource = type === 'external' ? statisticHealthStatisticsEx : statisticHealthStatisticsIn;

  const updateTaskId = (id: string | undefined) => {
    setTaskId(id);
    if (id) {
      sessionStorage.setItem("task_id_HealthStatistics", id);
    } else {
      sessionStorage.removeItem("task_id_HealthStatistics");
    }
  };
  ...
  const columns: IColumn<MStatistic.IHealthStatistics>[] = [
    {
    ...
    },
    {
      ...
      width: 100,
      align: 'center',
      render: (record: MStatistic.IHealthStatistics) => {
    ...
    {
      title: "Tỷ lệ bao phủ BHYT (%)",
      dataIndex: "bhyt_coverage_rate",
      width: 160,
      align: 'right',
    ...
    {
      title: "Số người tham gia tự nguyện",
      dataIndex: "voluntary_insured",
      width: 180,
      align: 'right',
      render: (val: number) => (val !== null ? val.toLocaleString() : "N/A"),
    },
  ...
  useEffect(() => {
    handleStatisticHealthStatistics(params)
  ...
  return (
    <>
      ...
      <Card
        title={<span style={{ color: token.colorPrimary }}>Thống kê số lượng cán bộ</span>}
      ...
      >
        <TableStaticData
          ...
          data={dataSource || []}
          addStt={true}
          loading={statisticHealthStatisticsLoading}
          hasTotal
          onReload={() => handleStatisticHealthStatistics(params)}
          otherButtons={[
            <>
              <Tooltip title={statusExtract === "COMPLETED" ? "Cập nhật dữ liệu mới" : "Đang cập nhật dữ liệu"}>
                <Button
                  onClick={async () => {
                    const res = await handleExtractHealthStatistics();
                    if (res?.task_id) {
...
export default StatisticHealthStatistics