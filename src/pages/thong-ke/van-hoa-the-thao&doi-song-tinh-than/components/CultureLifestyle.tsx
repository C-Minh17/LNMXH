...
const StatisticCultureLifestyle = () => {
  const { token } = theme.useToken()
  const { statisticCultureLifestyleEx, statisticCultureLifestyleIn, statisticCultureLifestyleLoading, handleStatisticCultureLifestyle, handleBackgroundTask, handleExtractCultureLifestyle } = useModel("statistic.statistic")
  const [statusExtract, setStatusExtract] = useState<string>()
  const [taskId, setTaskId] = useState<string | undefined>(() => {
    return sessionStorage.getItem("task_id_CultureLifestyle") || undefined;
  });

  const location = useLocation();
  const pathSegments = location.pathname.split('/');
  const type = pathSegments[2];
  const dataSource = type === 'external' ? statisticCultureLifestyleEx : statisticCultureLifestyleIn;

  const updateTaskId = (id: string | undefined) => {
    setTaskId(id);
    if (id) {
      sessionStorage.setItem("task_id_CultureLifestyle", id);
    } else {
      sessionStorage.removeItem("task_id_CultureLifestyle");
    }
  };
  ...
  const columns: IColumn<MStatistic.ICultureLifestyle>[] = [
    {
    ...
    },
    {
      ...
      width: 100,
      align: 'center',
      render: (record: MStatistic.ICultureLifestyle) => {
    ...
    {
      title: "Tổng số di tích",
      dataIndex: "total_heritage_sites",
      width: 140,
      align: 'right',
      render: (val: number) => (val !== null ? val.toLocaleString() : "N/A"),
    },
    {
      title: "Lượng khách du lịch",
      dataIndex: "tourist_visitors",
      width: 160,
      align: 'right',
      render: (val: number) => (val !== null ? val.toLocaleString() : "N/A"),
    },
    {
      ...
      dataIndex: "tourism_revenue_billion",
      width: 160,
      align: 'right',
      render: (val: number) => (val !== null ? val.toLocaleString('vi-VN', { minimumFractionDigits: 0, maximumFractionDigits: 2 }) : "N/A"),
    },
    {
      title: "Tăng dân số tự nhiên (%)",
      dataIndex: "natural_population_growth_rate",
      width: 180,
      align: 'right',
      render: (val: number) => (val !== null ? `${val}%` : "N/A"),
    },
    {
      title: "Tỷ lệ khám SK người cao tuổi (%)",
      dataIndex: "elderly_health_checkup_rate",
      width: 220,
      align: 'right',
      render: (val: number) => (val !== null ? `${val}%` : "N/A"),
    },
    {
      title: "Tỷ số giới tính khi sinh",
      dataIndex: "sex_ratio_at_birth",
      width: 180,
      align: 'right',
      render: (val: number) => (val !== null ? val.toLocaleString('vi-VN', { minimumFractionDigits: 1 }) : "N/A"),
    },
    {
      title: "Trạng thái",
  ...
  useEffect(() => {
    handleStatisticCultureLifestyle(params)
  ...
  return (
    <>
      ...
      <Card
        title={<span style={{ color: token.colorPrimary }}>Thống kê văn hóa & đời sống</span>}
      ...
      >
        <TableStaticData
          ...
          data={dataSource || []}
          addStt={true}
          loading={statisticCultureLifestyleLoading}
          hasTotal
          onReload={() => handleStatisticCultureLifestyle(params)}
          otherButtons={[
            <>
              <Tooltip title={statusExtract === "COMPLETED" ? "Cập nhật dữ liệu mới" : "Đang cập nhật dữ liệu"}>
                <Button
                  onClick={async () => {
                    const res = await handleExtractCultureLifestyle();
                    if (res?.task_id) {
...
export default StatisticCultureLifestyle